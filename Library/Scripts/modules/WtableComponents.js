function dContainer(container) {
  ...
  function dibujarTabla(tableID, config) {
      ...
      for (let z = 0; z < config.GroupDataset.length; z++) {//numeros encabezado
  ...
  function dibujarQuesito(configuracion){
  
              
              $("#canvasTalleresRealizados").html('');
  
              $("#"+configuracion.ContainerName).append('<div id="'+configuracion.ContainerName+'Div" style="width:50%;margin:auto;" >'+
                                                          '<canvas id="'+configuracion.ContainerName+'Grafica"></canvas>'+
                                                      '</div>')
              
              var ctx = document.getElementById(configuracion.ContainerName+"Grafica").getContext('2d');
              
              
  
              var arrayTotales = [];
              var arrayLabels = [];
              for (var i = 0; i < configuracion.Datasets.length; i++) {
                  console.log("En el índice '" + i + "' hay este valor: " + configuracion.Datasets[i]["cantidad"]);
                  arrayTotales.push(configuracion.Datasets[i]["cantidad"])
              }
              for (var i = 0; i < configuracion.Datasets.length; i++) {
                  console.log("En el índice '" + i + "' hay este valor: " + configuracion.Datasets[i]["time"]);
                  arrayLabels.push(configuracion.Datasets[i]["time"])
              }
              ...
              var config = {
                  ...
                  data: {
                      datasets: [{
                          data: arrayTotales,
                          backgroundColor: configuracion.Colors,
                          label: 'Dataset 1'
                      }],
                      labels: arrayLabels//configuracion.GroupDataset.time                    
                  },
                  options: {
                      responsive: true,
                      legend: {
